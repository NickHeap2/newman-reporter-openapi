...
const OpenAPICoverage = function (emitter, reporterOptions, collectionRunOptions) {
  ...
  const verboseMode = reporterOptions.verbose || false
  ...
  emitter.on('request', function (err, o) {
    ...
    utils.updateOperation(url, req.method, res.code, { verboseMode, serverUrl })
  })
  ...
  emitter.on('beforeDone', function (err, o) {
    ...
    cliReporter.logCoverage(coverage, openapiFilename)
